...
const CheckoutPage = () => {
    ...
    const { input, setInput } = useContext(ProductDataContext)
    ...
    }, [input.country, setisCountryValid])
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
    if(input.curtomerName ==='' || input.email ==='' || input.address ==='' || input.country ==='' || input.mobileNum ==='' || input.city ==='' || input.payment === ''){
    ...
    return (
        <div className='font-poppins lg:text-base text-sm'>
            ...
            <div>
                <h3 className='text-lg font-semibold text-start ml-9 text-yellow-600 '>Add Shipping Address:</h3>
            </div>


            <form action="" 
                className='text-center mt-[40px] pl-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 justify-around items-center gap-3'>
